...
public class CGFtoCNF {
			...
			for (int i = 0; i < productionList.size(); i++) {
				...
				for (int j = 0; j < temp.length(); j++) {
					...
					if (temp.length() == 3) {
						...
						for (int k = 0; k < keySet.size(); k++) {
							if (!keySet.get(k).equals(Character.toString(productionList.get(i).charAt(j)))) {
								found = false;
	...
	public String gen_comb(String a, String b) // creates every combination of variables from a and b . For eg: BA * AB
												// = {BA, BB, AA, BB}
	{
		String pri = a;
		String re = "";
	...
	public void trialRun() {
		...
		for (int i = 0; i < start.length(); i++)
			if (matrix[0][str.length() - 1].indexOf(start.charAt(i)) <= matrix[0][str.length() - 1].length()) // Checks
																												// if
																												// last
																												// element
																												// of
